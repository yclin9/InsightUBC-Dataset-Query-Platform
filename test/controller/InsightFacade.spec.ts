...
describe("InsightFacade", function () {
  ...
  async function makeRoomsDatasetZip(options?: {
    ...
    if (options?.invalidIndex) {
      zip.file(
        "index.htm",
        "<html><body><table><tr><td>bad row</td></tr></table></body></html>",
      );
      return zip.generateAsync({ type: "base64" });
    }
  ...
  before(async function () {
    ...
    invalidSection = await getContentFromArchives("invalid_section_folder.zip");
    validSection = await getContentFromArchives("valid_section_folder.zip");

    smallDataset = await getContentFromArchives("small_dataset.zip");
  ...
  describe("AddDataset", function () {
    ...
    it("should accept this valid id", async function () {
      ...
      expect(result).to.include("my-dataset");
    });

    it("should reject a rooms dataset with invalid zip content", async function () {
      try {
        await facade.addDataset("rooms", "not-a-zip", InsightDatasetKind.Rooms);
        expect.fail("Should have thrown!");
      } catch (err) {
        expect(err).to.be.an.instanceOf(InsightError);
      }
    ...
    it("should accept with valid section", async function () {
      ...
      expect(result).to.have.members(["my-dataset1", "my-dataset2"]);
    });
    ...
    it("should persist dataset across new InsightFacade instances", async function () {
      ...
      expect(datasets[0].id).to.equal("my-dataset");
    });
  });
  ...
  describe("ListDataset", function () {
    it("should return empty array when no dataset added", async function () {
      try {
        ...
        expect(datasetList).to.have.length(0);
      } catch (err) {}
    });
    ...
    it("should return an array with length of one when adding one dataset", async function () {
      try {
        ...
        expect(datasetList[0].id).to.equal("my-dataset");
        expect(datasetList[0].kind).to.equal(InsightDatasetKind.Sections);
        expect(datasetList[0].numRows).to.equal(1);
      } catch (err) {}
    });
    ...
    it("should return an array with length of two when adding two dataset", async function () {
      try {
        ...
        // VALIDATION
        expect(datasetList).to.be.an("array");
        expect(datasetList).to.have.length(2);
        expect(datasetList[0].id).to.equal("my-dataset1");
        expect(datasetList[1].id).to.equal("my-dataset2");
      } catch (err) {}
    });
  ...
  describe("PerformQuery", function () {
    ...
    it(
      "[invalid/not_exist.json] Query referencing non-existent dataset",
      checkQuery,
    );
    it(
      "[invalid/order_key_not_in_columns.json] Query with ORDER key not in COLUMNS",
      checkQuery,
    );
    ...
    it(
      "[valid/test_gt.json] SELECT dept, course, avg WHERE avg > 73",
      checkQuery,
    );
    it(
      "[valid/test_lt.json] SELECT dept, course, avg WHERE avg < 74",
      checkQuery,
    );
    it("[valid/test_eq.json] SELECT dept, avg WHERE avg = 78.69", checkQuery);

    it(
      '[valid/test_is.json] SELECT course, avg WHERE dept = "cpsc"',
      checkQuery,
    );
    it(
      '[valid/test_wildcard_start.json] SELECT dept, course, avg WHERE dept = "cp*"',
      checkQuery,
    );
    it(
      '[valid/test_wildcard_end.json] SELECT dept, course, avg WHERE dept = "*en"',
      checkQuery,
    );
    it(
      '[valid/test_wildcard_contain.json] SELECT dept, course, pass WHERE dept = "*t*"',
      checkQuery,
    );

    it(
      "[valid/test_no_filter.json] SELECT dept, course, no filter",
      checkQuery,
    );

    it(
      '[valid/test_and.json] SELECT dept, course, year WHERE avg > 73 AND Year = "2014"',
      checkQuery,
    );
    it(
      "[valid/test_or.json] SELECT dept, course, avg WHERE avg > 80 OR avg < 72",
      checkQuery,
    );
    it(
      '[valid/test_not.json] SELECT dept, course, avg WHERE dept IS NOT "cpsc"',
      checkQuery,
    );
    ...
    it(
      "[invalid/gt_value_not_number.json] GT value is not a number",
      checkQuery,
    );
    it("[invalid/gt_not_object.json] GT value is not an object", checkQuery);
    it(
      "[invalid/group_apply/missing_group.json] TRANSFORMATIONS missing GROUP",
      checkQuery,
    );
    it(
      "[invalid/group_apply/group_empty.json] GROUP is empty array",
      checkQuery,
    );
    it(
      "[invalid/invalid_column_key.json] COLUMNS key with invalid format",
      checkQuery,
    );
    it(
      "[invalid/group_apply/group_invalid_field.json] GROUP key references invalid field",
      checkQuery,
    );
    ...
    // Sorting
    it(
      "[invalid/sorting/invalid_order_dir.json] Invalid ORDER direction",
      checkQuery,
    );
    it(
      "[invalid/sorting/order_missing_dir.json] ORDER object missing dir",
      checkQuery,
    );
    it(
      "[invalid/sorting/order_missing_keys.json] ORDER object missing keys",
      checkQuery,
    );
    it("[valid/sorting/test_sort_up.json] Sort results ascending", checkQuery);
    it(
      "[valid/sorting/test_sort_down.json] Sort results descending",
      checkQuery,
    );
    it(
      "[valid/sorting/test_sort_multi.json] Sort by multiple keys",
      checkQuery,
    );

    // GROUP/APPLY
    it(
      "[invalid/group_apply/columns_not_in_group_or_apply.json] COLUMNS are not included in GROUP or APPLY",
      checkQuery,
    );
    it(
      "[invalid/group_apply/invalid_apply_key.json] Invalid APPLY key",
      checkQuery,
    );
    it(
      "[invalid/group_apply/duplicate_apply_key.json] Duplicate APPLY key",
      checkQuery,
    );
    it(
      "[invalid/group_apply/avg_on_sfield.json] AVG used on string field",
      checkQuery,
    );
    it(
      "[valid/group_apply/test_avg_rounding.json] GROUP by dept and AVG avg result (rounded to two decimal places)",
      checkQuery,
    );
    it(
      "[valid/group_apply/test_sum_rounding.json] GROUP by dept and SUM avg result (rounded to two decimal places)",
      checkQuery,
    );
    it(
      "[valid/group_apply/test_max.json] GROUP by dept and MAX avg result",
      checkQuery,
    );
    it(
      "[valid/group_apply/test_min.json] GROUP by dept and MIN avg result",
      checkQuery,
    );
    it(
      "[valid/group_apply/test_count.json] GROUP by dept and COUNT unique values",
      checkQuery,
    );
